...
#ifdef LESS_MACROS
uint8 HAS_N_OBJECT_FIELDS(uint16 visit)
{
	return (RAM_VECTOR_P(visit));
}
...
#else
#define HAS_N_OBJECT_FIELDS(visit) (RAM_VECTOR_P(visit))
#define HAS_2_OBJECT_FIELDS(visit) (RAM_PAIR_P(visit) || RAM_CONTINUATION_P(visit))
#ifdef CONFIG_BIGNUM_LONG
#define HAS_1_OBJECT_FIELD(visit)  (RAM_SYMBOL_P(visit) \
				    || RAM_CLOSURE_P(visit) || RAM_BIGNUM_P(visit))
#else
#define HAS_1_OBJECT_FIELD(visit)  (RAM_SYMBOL_P(visit) || RAM_CLOSURE_P(visit))
#endif
#endif
